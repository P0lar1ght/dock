...
/// TTY/session detach copied from grok `xai_tty_utils::detach_std_command`
/// (`setsid` on Unix). Used by `mmdc` so Chromium grandchildren can be reaped.
pub fn detach_std_command(cmd: &mut Command) {
    #[cfg(unix)]
    {
...
/// Pager-safe child env (no TTY takeover). Grok's helper also sets this.
pub fn pager_env() -> Vec<(std::ffi::OsString, std::ffi::OsString)> {
    Vec::new()
}
...
mod tests {
    ...
    fn detached(mut cmd: Command) -> Command {
        cmd.stdin(Stdio::null())
            .stdout(Stdio::null())
            .stderr(Stdio::null());
        super::detach_std_command(&mut cmd);
        cmd
    }
