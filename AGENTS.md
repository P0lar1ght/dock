...
| 回环网关 | `gateway` | `"gateway"`（`GatewayRef`）；事件 `gateway/pairing`。只绑 loopback（默认 `127.0.0.1:18991`，同时尝试 `[::1]` 同端口；`DOCK_GATEWAY_BIND` 可覆盖）。CORS 反射 Origin 是有意的：鉴权靠配对 + 回环，不是 Origin 白名单。Approved 的 poll **不**回 ticket 明文，浏览器走 `POST /v1/pairing/exchanges` |
