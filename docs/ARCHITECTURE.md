...
8. **Gateway 默认不监听。** 只绑 loopback（首选 `127.0.0.1:18991`，占用往上找，同端口再试 `[::1]`；`DOCK_GATEWAY_BIND` 只改首选）。`/pair` 开启；鉴权靠配对 + 一次性 ticket + 回环，CORS 反射 Origin 是有意的。 **例外是 `dock serve`**：由父进程拉起，挂载即监听（默认 `127.0.0.1:0`），ticket 由 `PairingStore::issue_trusted` 签出、**只经 stdout 管道**交给父进程——信任锚是「读得到这个进程 stdout 的人」。它不留 Origin 绑定，所以 `/v1/connection/tickets` 仍要配对；不要把受信签发接到 HTTP 上（Origin 在浏览器之外可以伪造）。
