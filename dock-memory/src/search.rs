...
pub fn hybrid_search_merge(
    ...
    // Drop content-free candidates *before* normalization. The generated
    // `MEMORY.md` indexes are scaffold text that `is_content_free` rejects
    // further down anyway, but if they take part in the rank range they sit at
    // the top and squeeze every real note toward 0 — a query whose best
    // candidates are all scaffold (e.g. `Dock`) then returns nothing at all.
    fts_results.retain(|r| {
        index
            .get_chunk(&r.chunk_id)
            .ok()
            .flatten()
            .is_some_and(|chunk| !is_content_free(&chunk.text, &chunk.source))
    });
...
mod tests {
    ...
    /// Regression: a content-free scaffold chunk used to join the rank
    /// normalization before being filtered out. Being the best match, it took
    /// normalized 1.0, so the real note sharing the keyword was pushed to ~0.0
    /// and dropped by `FTS_ONLY_MIN_SCORE` — the query returned nothing even
    /// though the note was indexed.
    #[test]
    fn scaffold_chunk_does_not_suppress_real_notes() {
        let tmp = tempfile::tempdir().unwrap();
        let root = MemoryRoot::open(tmp.path(), Path::new("/tmp/scaffold-rank"));
        root.ensure_layout().unwrap();
        // Stands in for the generated MEMORY.md: scaffold text, keyword-heavy
        // and short, which makes it the strongest BM25 match.
        std::fs::write(
            root.workspace.topics.join("generated-index.md"),
            "# Workspace memory\n\n> Generated by Dock. Do not edit this file directly.\n\n\
             - dock dock dock dock\n",
        )
        .unwrap();
        std::fs::write(
            root.workspace.topics.join("real-note.md"),
            "## Keybindings\n\nThe Dock TUI ships overlays, a scrollback and a prompt.\n",
        )
        .unwrap();
        let mut idx = MemoryIndex::open_or_create(&root.search_db()).unwrap();
        idx.reindex_tree(&root).unwrap();

        let hits = search_memory(&root, "dock", 5).unwrap();
        assert!(
            !hits.is_empty(),
            "real note must survive scaffold ranking; hits={hits:?}"
        );
        assert!(
            hits.iter().any(|h| h.text.contains("Keybindings")),
            "expected the real note, not just scaffold; hits={hits:?}"
        );
    }
}
