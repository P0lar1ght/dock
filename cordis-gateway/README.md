...
| `preset/get { id }` | 编辑器用的整份定义：列表那几项外加 `name` / `order` / `persona` / `replacePrompt` / `tools`（`null` = 全部已注册工具，`[]` = 不用工具）/ `agents[]`（`id` / `name` / `description` / `persona` / `tools` / `replacePrompt` / `listings` / `builtin`：内置预设自带的角色）/ `path`（落盘的 `agent.yml`，内置没改过为 `null`）。损坏的预设照样回，`available: false` + `error` |
| `preset/update { id, preset }` | 整份写回（字段同 `preset/get`）。内置预设写成 `~/.dock/presets/<id>/` 覆盖层；名册里去掉的角色删掉它的文件，内置自带的角色删不掉；损坏的预设整份重写。校验不过（名为空、整份替换却没有提示词、角色 id 不合法或重复、图标名不合法）回 `invalid_params`、不写盘 |
| `tool/catalog` | 预设能选的工具（同 TUI `/preset` 画布左栏，不含 MCP）：`name` / `summary`（描述第一句）/ `kind`（`resident` / `deferred` 按需 / `dynamic` 运行中的动态包，不受允许名单限制） |
