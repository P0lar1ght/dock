...
fn send_initial_configuration(
    ...
    if let Some(ref settings) = config.settings {
        if let Err(e) = server.did_change_configuration(lsp_types::DidChangeConfigurationParams {
...
impl LspClient {
    ...
    > {
        ...
        super::process::detach_std_command(&mut cmd);
        cmd.envs(super::process::pager_env());
    ...
    pub fn notify_file_change(
        ...
        if let Some(saved) = self.policy.did_save(uri.clone(), content) {
            if let Err(e) = self.socket.did_save(saved) {
