...
/// Grok's `DomainNotAllowed::DomainNotAllowed` — kept local so domain.rs stays
/// copy-close without the rest of the Grok output enum.
#[derive(Debug, Clone, PartialEq, Eq)]
pub enum DomainNotAllowed {
...
impl DomainMatcher {
    ...
    /// Returns `None` if the URL is permitted, or `Some(DomainNotAllowed)`
    /// if it should be blocked. When `entries` is empty, all fetches are blocked.
    pub fn check(&self, url: &Url) -> Option<DomainNotAllowed> {
        let Some(raw_host) = url.host_str() else {
            return Some(DomainNotAllowed::DomainNotAllowed(String::new()));
        };
        ...
        match self.entries.get(&host) {
            ...
            Some(HostEntry::PathPrefixes(prefixes)) => {
                ...
                Some(DomainNotAllowed::DomainNotAllowed(host))
            }
            None => Some(DomainNotAllowed::DomainNotAllowed(host)),
        }
    }
}
...
mod tests {
    ...
    fn rejects_unlisted_domain() {
        ...
        assert!(matches!(blocked, Some(DomainNotAllowed::DomainNotAllowed(d)) if d == "evil.com"));
    }
    ...
    fn path_scoped_rejects_sibling_prefix() {
        ...
        assert!(
            matches!(blocked2, Some(DomainNotAllowed::DomainNotAllowed(h)) if h == "vercel.com"),
