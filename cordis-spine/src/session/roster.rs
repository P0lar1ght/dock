...
//!
//! `Sessions::archived()`（走 [`persist::load_cwd`]）只看当前工作目录那
...
impl Roster {
    ...
    /// 走这条而不是让 TUI 直接调 `persist::remove`：删完必须 invalidate，
    /// 两步绑在一起才不会出现「删了但列表还在」。
    pub fn remove(&self, id: &str, cwd: &std::path::Path) -> std::io::Result<()> {
        let result = persist::remove(id, cwd);
        self.invalidate();
        result
    }
}
...
mod tests {
    ...
    fn newest_first() {
        ...
        for (id, updated) in [("old", 1_000u64), ("new", 3_000), ("mid", 2_000)] {
            persist::save(&archive(id, id, vec![LogEvent::User(id.into())]), cwd).unwrap();
            let meta = persist::sessions_cwd_dir(cwd).join(id).join("meta.json");
            std::fs::write(
                &meta,
                format!(
    ...
    fn summary_reads_the_tail_of_a_large_transcript() {
        ...
        persist::save(&archive("big", "大会话", events), cwd).unwrap();

        let history = persist::sessions_cwd_dir(cwd)
            .join("big")
