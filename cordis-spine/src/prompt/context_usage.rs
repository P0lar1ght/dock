...
fn workflows_detail(snap: &ContextSnapshot) -> OccupancyDetail {
    let tokens = snap
        ...
        .map(|c| c.tokens)
        .unwrap_or(0);
    let listing = crate::tools::workflow::catalog_listing();
...
fn instructions_detail(ctx: &Context, snap: &ContextSnapshot) -> OccupancyDetail {
    let tokens = snap
        ...
        .map(|c| c.tokens)
        .unwrap_or(0);
    let rows: Vec<DetailRow> = crate::prompt::project_instructions::instruction_paths()
        .into_iter()
    ...
    OccupancyDetail {
        ...
        text: crate::prompt::project_instructions::injected_copy(ctx),
    }
}
...
fn skills_detail(ctx: &Context, snap: &ContextSnapshot) -> OccupancyDetail {
    ...
    let rows: Vec<DetailRow> = ctx
        .get::<crate::tools::skills::Skills>(SKILLS)
        .map(|s| {
            s.occupancy_rows()
                .into_iter()
...
"#,
    ...
    async fn messages_detail_breaks_out_search_tool_cost() {
        ...
        sessions.append(LogEvent::ToolExecute {
            id: "call-1".into(),
            name: crate::tools::mcp::SEARCH_TOOL_NAME.into(),
    ...
    async fn detail_instructions_lists_the_layers_and_the_injected_copy() {
        ...
        let body = crate::prompt::project_instructions::render(&ctx).expect("rules");
