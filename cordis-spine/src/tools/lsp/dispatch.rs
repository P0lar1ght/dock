...
async fn bootstrap_lsp(
    ...
    for name in restartable {
        ...
        tokio::spawn(crate::tools::lsp::restart_monitor(mgr_weak, name));
    }
    Ok(())
}
...
mod tests {
    use super::*;
    use crate::tools::lsp::config::{command_on_path, LspServerConfig};
    use crate::tools::lsp::notify::ToolNotificationHandle;
    use crate::tools::lsp::{LspBackend, LspOperation, LspToolInput};
    ...
    async fn rust_analyzer_hover_when_on_path() {
        ...
        let mgr = Arc::new(tokio::sync::Mutex::new(crate::tools::lsp::LspManager::new(
            servers,
