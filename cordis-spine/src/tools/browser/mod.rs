...
use crate::tools::registry::{
    own_registered, tool_result, tool_result_with_images, ToolBody, Tools,
};
    ...
    match result {
        Ok(content) => {
            if call.name == "browser_screenshot" {
                ...
                if let Some(img) =
                    crate::tools::tool_images::user_image_from_path(std::path::Path::new(path))
                {
                    images.push(img);
                }
...
mod tests {
    ...
    use crate::tools::registry::{tools, Tools};
    use cordis::Context;
    use std::path::Path;
