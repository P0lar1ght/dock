...
impl WorkflowRegistry {
    pub(crate) fn scan(session_cwd: Option<&Path>) -> Self {
        ...
        if let Some(cwd) = session_cwd {
            dirs.push((project_root(cwd).join(".dock").join("workflows"), "project"));
        }
...
fn bundled_file_is_managed(_path: &Path) -> bool {
    false
}
...
pub(crate) fn resolve_by_path(
    ...
    let canonical = std::fs::canonicalize(&candidate).map_err(|error| ResolveError::Io {
        path: candidate.display().to_string(),
        error: error.to_string(),
    })?;

    let project = std::fs::canonicalize(project_root(session_cwd)).ok();
    let user_workflows = std::fs::canonicalize(user_workflow_dir()).ok();
    ...
    if !in_project && !in_user_or_session {
        return Err(ResolveError::UntrustedPath {
            path: candidate.display().to_string(),
            reason: "outside the project, dock home, and session workflow runs".into(),
        });
    }
...
pub(crate) fn save_project_workflow(
    ...
    let canonical_root = std::fs::canonicalize(&root).map_err(|error| ResolveError::Io {
        path: root.display().to_string(),
        error: error.to_string(),
    })?;
    let dir = canonical_root.join(".dock").join("workflows");
    create_contained_workflow_dir(&canonical_root, &dir)?;
    let canonical_dir = std::fs::canonicalize(&dir).map_err(|error| ResolveError::Io {
...
fn create_contained_workflow_dir(root: &Path, dir: &Path) -> Result<(), ResolveError> {
    let root = std::fs::canonicalize(root).map_err(|error| ResolveError::Io {
        path: root.display().to_string(),
        error: error.to_string(),
    })?;
    let relative = dir
    ...
    let canonical = std::fs::canonicalize(dir).map_err(|error| ResolveError::Io {
        path: dir.display().to_string(),
        error: error.to_string(),
    })?;
...
mod tests {
    ...
    #[test]
    fn scan_project_dock_workflows_skips_filename_mismatch() {
        ...
        assert!(matches!(
            resolve_by_path(&wf_dir.join("wrong.rhai"), dir.path(), None),
