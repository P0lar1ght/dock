...
Mermaid 布局栈在 [`dock-render/third_party/`](../dock-render/third_party/)（不再放本目录）。

## 许可证

沿用上游许可证，不要改成第一方的 `cordis-*` 那套：`xai/` 多数目录是 Apache-2.0。声明在各 crate 的 `Cargo.toml`，Apache-2.0 副本放在对应目录的 `LICENSE`。替换或升级目录时保持这些文件与上游一致。
