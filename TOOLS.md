...
5. **扩展走 waterfall**（`tools/execute`、`system-prompt/assemble`、`agent/turn-end`、`agent/step-start` 等），不要在 loop 里分支。「模型出了文本但还不该收尾」就是 `agent/turn-end`，「跑到一半要提醒一句」就是 `agent/step-start`，不是往 `grok_sample_loop` 里再加一个 `if` 或一份计数状态。
