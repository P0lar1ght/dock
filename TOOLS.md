...
| `tool-scheduler` | → `"tools"`（live `"cron"`） | `scheduler_create` `scheduler_list` `scheduler_delete` | 包着已有 `"cron"` |
...
- MCP：stdio + Content-Length；无 HTTP MCP、无 elicitation
