...
| `tool-browser` | `"browser"` + `"tools"` | `browser_open` `browser_snapshot` `browser_click` `browser_type` `browser_screenshot` `browser_tabs` `browser_close`（按需） | **骨架（PR A）**：named `"browser"` 会话态 `Closed`，不驱动 Chromium。`register_deferred`：不进 sampler / `specs_for_model`，模型只经 `search_tool` + `use_tool` 发现和调用。调用返回未连接（PR B 才接 CDP）。无 chromiumoxide / Node。TUI `/browser` 是 `"slash"` extra overlay，报「未连接」。`code` / `cordis` 允许名单含这七项；`minimal` / `warden` 主代理不含 |
