...
| `tool-browser` | `"browser"` + `"tools"` | `browser_open` `browser_navigate` `browser_navigate_back` `browser_snapshot` `browser_click` `browser_hover` `browser_type` `browser_press_key` `browser_select_option` `browser_fill_form` `browser_wait_for` `browser_screenshot` `browser_tabs` `browser_close`（按需） | **BUA D1 / P0**：in-process **chromiumoxide** CDP。`browser_open` = 确保会话 + 首个 URL（可懒启动独立 `$DOCK_HOME/browser/user-data` Chromium）；`browser_navigate` / `browser_navigate_back` = 已有会话内跳转（Closed 时报错，不启动）。另有 hover / press_key / select_option / fill_form / wait_for。`browser_snapshot` 出 lean a11y 树 + `@eN` refs（agent-browser 仅作形状参考，非运行时依赖）。无 Node/Playwright。`register_deferred`：不进 sampler / `specs_for_model`。Fiber dispose 关掉 Chromium 并注销 tools/slash/named service。`/browser` 是完整 TUI 驾驶舱（未连接/已连接 + 标签页 + 最近截图 + 审批；不渲染网页）。`code` / `cordis`（+ general-purpose）允许名单含这些 `browser_*`；`minimal` / `warden` 主代理不含 |
