...
The memo map above is the body of `source.rhai` (preferred) or a tiny inline `source`. After define returns `memo-N` / `pkg-N`, call `cordis_run` with `mode: "run"`. While the Package is running, `memo_get` / `memo_set` are on-demand tools: hidden from the sampler, found with `search_tool` and called with `use_tool`, and callable even if the Agent preset YAML omitted them. Verify immediately in the same turn with `cordis_call` `name: "memo_get"` (arguments `{}`). `cordis_stop` unregisters tools, slash, slots, and the bag.
...
| `host.on("session/event", \|line\| { ... })` | — | Observe the session log after append. `line` is `user\t…` / `assistant\t…` / `tool\tname` / `reminder\t…` / `turn-end\tcompleted|cancelled|failed` (one per finished turn). Runs asynchronously; do not treat it as a waterfall. Stop unregisters. Do not re-enter `Sessions` from the handler |
