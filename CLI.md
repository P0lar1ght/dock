...
| `/browser` | 额外命令（`"slash"` extras，不是 CATALOG）：Notice 驾驶舱（live-lookup `"browser"`）。显示 Closed/Connected、标签页缓存、最近截图路径、断开提示；不渲染网页。BUA 由 chromiumoxide CDP 驱动；`browser_*` 走 `search_tool` / `use_tool`，不进默认 sampler 表 |
