...
| 滚动区 | 用户气泡灰带铺满行宽（Grok `with_background`）；图标抄 Grok `todo_pane`（`□` `▶` `✓` `✗`）。**`subagent` 是独立卡片**：当前模式名册角色名 + type id，始终折叠，点击打开全屏对话。Grok `task` 仍画成原来的子代理块。后台 bash / `monitor` 画成任务卡片。前台 `bash` 画成 **Bash 卡**：标题 `◆ Bash $ <命令>`（工具名粗体 + dim 的 `$` + 命令，与 `Read` / `Glob` / `List` / `Search` / `Edit` 同构；原先只有一个 `$` 没有工具名，扫一眼分不出是什么卡）。**相邻的多次前台 bash 合成一张卡**，标题 `◆ Bash N 条命令`，展开后逐条列 `$ 命令` + 各自输出（空输出留「（无输出）」，读得出跑过没有）。中间夹了任何别的事件就断开——合并的前提是它们视觉上本来连成一片；后台 bash 归任务卡（自带任务 id / 实时输出 / 时钟）、正在跑的调用要单独挂时钟，都不参与合并。**组内每条命令各自可折**：组头点击切整组（折叠键 `bash-group:<首条 id>`），每条命令的 `$ 命令` 与正文归它自己的 tool call id，点它只折它——三条长输出可以只收掉其中一条。这正是 grok 的形态（每条 execute 调用各是一个带 `DisplayMode` 的块），只是外观上收进一张卡。每条命令缺省继承组的折叠态，单独点过才有自己的值。`update_goal` 画成 **Goal 卡**（设定 / 进展 / 完成 / 受阻）。`scheduler_create` / `scheduler_list` / `scheduler_delete` 画成 **Loop 卡**（设定 / 列表 / 关闭）。`search_tool` 画成 **Search Tools** 卡（关键词 + 条数；展开为「动作  服务器」列表，不把 JSON schema 铺开）。MCP 调用（`use_tool` 或遗留 `mcp_{server}__{tool}`）画成 **Server Action** 卡（参数 kv + 输出；`use_tool` 标题用内层 `tool_name`）。所有工具卡共用一套外壳（`scrollback/card.rs`）：正文一律缩进 2 列（与 `◆ ` 等宽，正文左边缘和标题文字对齐）、换行宽度同一个常量、截断脚注统一成「`… 还有 N 行`／`N 项`／`N 个文件`」、标题行末尾钉折叠符（`›` 收起、`⌄` 还有更多、`⌃` 全展开），**整张卡都可点**，不只是折叠头那一行。打开覆盖层的卡片（子代理 / 后台任务）不折叠，仍写「（点击查看）」。运行中的卡片**头行扫光**：一道高光带拖尾从左扫到右，把文字染成该卡的状态色（工具青、思考紫），扫完停一拍再来，行尾右对齐一个实时耗时。扫光和耗时都在 **paint 期**画，不进滚动区的布局缓存——卡片正文本身不含任何会自己变的内容（长跑 bash / 子代理干活时父会话不追加事件，烤进去的动画会直接冻住） |
