...
| `/browser` | 额外命令（`"slash"` extras，不是 CATALOG）：完整 TUI 驾驶舱 overlay（PR C；live-lookup `"browser"`）。显示未连接/已连接、标签页缓存、最近截图路径、审批提示；不渲染网页。BUA 由 chromiumoxide CDP 驱动；`browser_*` 仍 `register_deferred`（不进 sampler / `specs_for_model`），走 `search_tool` / `use_tool` |
