...
| `/browser` | 额外命令（`"slash"` extras，不是 CATALOG）：完整 TUI 驾驶舱 overlay（live-lookup `"browser"`）。显示未连接/已连接、标签页缓存、最近截图路径、最近 wait_for、P0 能力摘要、审批提示；不渲染网页。BUA 由 chromiumoxide CDP 驱动；`browser_open` 确保会话+首 URL，`browser_navigate`/`browser_navigate_back` 仅在已连接会话内跳转；另有 hover/press_key/select/fill/wait。`browser_*` 仍 `register_deferred`（不进 sampler / `specs_for_model`），走 `search_tool` / `use_tool` |
