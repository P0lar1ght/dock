...
| `/browser` | 额外命令（`"slash"` extras，不是 CATALOG）：薄状态 Notice（未连接/已连接）。BUA 驾驶舱由 chromiumoxide CDP 驱动（PR B）；完整 TUI 见 PR C。模型工具 `browser_*` 走 `search_tool` / `use_tool`，不进默认 sampler 表 |
